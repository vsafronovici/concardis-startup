...
const SliderComponent = props =>  {
    ...
    const checkOptions = (type === FieldType.DROPDOWN) 
                ? values(options.dropdownItems).map( selectValue => {
                  return translate(selectValue)
                })
                : options.sliderValues.map( sliderValue => {
                      return Number(translate(sliderValue))
                   })
        ...
        <div className="sc-content">
          ...
          <div className={(type === FieldType.DROPDOWN) ? 'sc-slider-select' : 'sc-slider-slider'}>
            {(type === FieldType.DROPDOWN)
              ? <RenderSelect selectItems={checkOptions} name={name} title={translate(title)} handleChangeField={handleChangeField} defaultValue={translate(defaultValue)} fieldHint={translate(fieldHint)}/>
              : <RenderSlider sliderItems={checkOptions} name={name}  title={translate(title)} defaultValue={Number(translate(defaultValue))} handleChangeField={handleChangeField}/> }
          </div>
        </div>
    )
}
